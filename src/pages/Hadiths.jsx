...
import { checkIfTrialEnd, createHadithAppActivation, getBooks } from "../database/hadithRepository";
...
export default function Hadiths(){
    ...
    const goToBook= async(nameEnglish, name, link)=>{
        
        await localforage.setItem("hadithItem", {nameEnglish, name});
        navigate(link)
    }
                    ...
                    <div className="flex gap-2 flex-wrap justify-center p-4">                    
                        {
                            ...
                            hadiths.map(item=>{
                                return(
                                    <div onClick={()=>goToBook(item.nameEnglish, item.name, item.link)} 
                                         class="w-full flex flex-col items-start bg-neutral-primary-soft p-6 border-t border-default rounded-base shadow-xs md:flex-row md:max-w-sm md:flex-row md:max-w-sm">
