    ...
    const getSubjectiveHadiths=async(searchHadith)=>{
        ...
        try{
            ...
            if (Capacitor.isNativePlatform()) {
                ...
                const result = await db.query(`SELECT distinct bookId, bookName, chapterId, chapterTitle, titleId, chapterTitleIndexName FROM subjectives
                ...
                const result = db.exec(`SELECT distinct bookId, bookName, chapterId, chapterTitle, titleId, chapterTitleIndexName  FROM subjectives
                                                WHERE chapterTitleIndexName LIKE '%${searchHadith}%' `);
    ...
    return(
        <>
            ...
            <form onSubmit={(e)=>{
                ...
                <label><Link to='/activationCompo'>বিষয় দিয়ে খুঁজুন:</Link> (যেমন: ঈমান, সালাত/নামায, রোযা, যাকাত, হজ্জ, উযূ/ওজু, গোসল, তায়াম্মুম, বিবাহ/বিয়ে, তালাক, হায়েয, হালাল, হারাম, মাকরূহ, সুদ ...)  </label>
                ...
                <button type="submit" class="rounded-md bg-cyan-900 px-4 py-2 text-white mt-2 mb-2">Search</button>                 
            </form>                              
                                ...
                                return(
                                    <Link to ={item.link} class="w-full flex flex-col items-start bg-neutral-primary-soft p-6 border-t border-default rounded-base shadow-xs md:flex-row md:max-w-sm md:flex-row md:max-w-sm">
                                        <div class="flex flex-col justify-between md:p-4 leading-normal">
                                            <h5 class="mb-2 text-2xl font-semibold tracking-tight text-heading">{item.chapterTitleIndexName} - {item.bookName} (পড়ুন)</h5>
                                        </div>
                                    </Link>                            
                                )
                            })
